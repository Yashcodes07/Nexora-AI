      ...
      {!checkingIn ? (
        ...
        <main className="checkin-chat">
          ...
          <section className="checkin-chat__messages" aria-live="polite">
            ...
            {messages.map((item) => <div className={`checkin-message checkin-message--${item.from}`} key={item.id}>{item.from === "nexora" && <span className="checkin-message__mark">N</span>}<p>{item.text}</p></div>)}
          </section>
